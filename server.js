...
app.get('/api/status', (req, res) => {
  res.json({ status: 'ok', message: 'BitSolHunter backend is 
alive' });
});
...
app.post('/verify-payment', async (req, res) => {
  ...
  try {
    ...
    if (!heliusApiKey) {
      return res.status(500).json({ error: 'Helius API key not 
configured' });
    }
...
parseFloat(transfer.amount) >= 2039280 // Example threshold in 
lamports (~$30 SOL)
      )
    );
    ...
    const usdtPayment = transactions.some(tx =>
      tx.tokenTransfers && tx.tokenTransfers.some(transfer =>
        ...
        transfer.mint === 
"Es9vMFrzrXyKbnntR1PdhHb9jdqkV6ttcKxZjYzR8jga" &&
        parseFloat(transfer.rawTokenAmount.tokenAmount) >= 30 * 10 
** 6
      )
    );
